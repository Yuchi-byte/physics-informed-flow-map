...
def psi_t(
    x_0: torch.Tensor, x_1: torch.Tensor, t: torch.Tensor, sig_min: float = sig_min
) -> torch.Tensor:
...
def logit_normal_map(t: torch.Tensor, m: float = 0.0, s: float = 1.0) -> torch.Tensor:
    ...
    return (
        t
        * torch.exp(-0.5 * torch.square((torch.log(t / (1 - t)) - m) / s))
...
def cfm_loss_residual(
    ...
    **kwargs,
    ...
    # kept stepping through time until we reach the clean data x_1_pred.
...
def sample(
    ema, x_0: torch.tensor, num_steps: int, use_stoc_samp: bool, **kwargs
) -> torch.tensor:
    ...
    for t in torch.arange(0, num_steps, device=x_0.device):
        if time_steps[t] < 0.2 and use_stoc_samp:
            # when t is very close to noise (rather than the clean data), we add more noise to x_new.
            x_new += (1 - time_steps[t]) * ema(
                x_new, time_steps[t].unsqueeze(0), **kwargs
            )
            x_new = (1 - time_steps[t + 1]) * torch.randn_like(x_new) + time_steps[
                t + 1
            ] * x_new
        else:
            x_new += (time_steps[t + 1] - time_steps[t]) * ema(
                x_new, time_steps[t].unsqueeze(0), **kwargs
            )
    # kept stepping through time until we reach the clean data.
    return x_new
