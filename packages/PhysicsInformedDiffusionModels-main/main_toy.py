...
config = {
    ...
    "create_gif": True,  # Create GIFs of denoising process for visualization (takes some time)
...
for t in pbar:  # for every training step.
    permutation = torch.randperm(dataset.size()[0])
    for i in range(0, dataset.size()[0], config["batch_size"]):  # for every batch.
        ...
        loss, data_loss, residual_loss, ineq_loss, opt_loss = model_estimation_loss(
            ...
            config["n_steps"],
            diff_dict,
            model_pred_mode=config["model_pred_mode"],
            ...
            use_ddim_x0=config["use_ddim_x0"],
    ...
    if t % config["sample_freq"] == 0:
        shape = [config["no_samples"], config["dim"]]  # [1000, 2]
        seqs = p_sample_loop(
            model,
            shape,
            config["n_steps"],
        ...
        fig, axs = plt.subplots(
            1, config["tot_eval_steps"], figsize=(3 * config["tot_eval_steps"] - 3, 3)
        )
            ...
            if seq:
                for i_idx, i in enumerate(eval_steps):
                    ...
                    if config["fix_axes"] and seq_idx == 0 and i_idx == 0:
                        x_lim, y_lim = (
                            (cur_x[:, 0].min(), cur_x[:, 0].max()),
                            (cur_x[:, 1].min(), cur_x[:, 1].max()),
                        )
                    axs[i_idx].set_xlim(x_lim)
                    axs[i_idx].set_ylim(y_lim)
                    axs[i_idx].scatter(
                        cur_x[:, 0], cur_x[:, 1], s=10, label=labels[seq_idx]
                    )
                    axs[i_idx].set_title(
                        f"$q(\\mathbf{{x}}_{{{config['n_steps'] - i}}})$"
                    )
                    if i_idx == 0:
                        axs[i_idx].legend()
                ...
                if seq_idx == 0:
                    ...
                    np.savetxt(
                        f"{output_save_dir}/csv/step_{t}_{labels[seq_idx]}.csv",
                        seq[-1].detach().cpu(),
                        delimiter=",",
                    )
                ...
                if config["create_gif"]:
                    ...
                    array_to_gif(
                        seq,
                        f"{output_save_dir}/step_{t}_{labels[seq_idx]}.gif",
                        x_lim=x_lim,
                        y_lim=y_lim,
