...
from mfm.utils.evaluation import plot_posterior_samples, posterior_sampling_fn
...
class TrainingModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ema_val_loss, ema_val_aux_losses = self.loss_fn(
            self.model,
            self.weighting_model,
            x,
            labels,
...
class SamplingCallback(Callback):
    ...
    def setup(self, trainer, pl_module, stage=None):
        ...
        self.shared_unconditional_noise = self._create_shared_tensor(
            (self.cfg.sampling.n_unconditional_samples, *self.image_shape), g
        ).to(device)
        self.shared_noise = self._create_shared_tensor(
            (self.cfg.sampling.n_conditioning_samples, *self.image_shape), g
        ).to(device)
        self.shared_posterior_noise = self._create_shared_tensor(
            (
                self.cfg.sampling.n_conditioning_samples
                * self.cfg.sampling.n_samples_per_image,
                *self.image_shape,
            ),
            g,
        ).to(device)
    ...
    def _run_distributed_sampling(self, pl_module, trainer):
        ...
        all_indices = torch.arange(
            self.cfg.sampling.n_unconditional_samples, device=device
        )

        # 1) unconditional sampling w/ CFG
        for n_steps in tqdm.tqdm(
            self.cfg.sampling.n_kernel_steps,
            desc="Unconditional sampling with CFG at different n_steps",
        ):
            for cfg_scale in self.cfg.sampling.kernel_cfg_scales:
                with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                    device_indices = all_indices[self.rank :: self.world_size]
                    unconditional_noise_device = self.shared_unconditional_noise[
                        device_indices
                    ]
                    unconditional_labels_device = self.shared_unconditional_labels[
                        device_indices
                    ]
                ...
                unconditional_samples_kernel = self._decode(
                    pl_module,
                    unconditional_samples_kernel,
                    vae_batch_size=self.cfg.sampling.vae_batch_size,
                )

                unconditional_samples_kernel = self._gather(
                    unconditional_samples_kernel, device_indices, trainer
                )
                if trainer.is_global_zero:
                    unconditional_samples_kernel = unconditional_samples_kernel.clamp(
                        0.0, 1.0
                    )
        ...
        all_indices = torch.arange(
            self.cfg.sampling.n_conditioning_samples, device=device
        )
        device_indices = all_indices[self.rank :: self.world_size]
        ...
        for t_cond in tqdm.tqdm(
            self.cfg.sampling.consistency_sampler.t_conds,
            desc="Consistency sampling at different t_conds",
        ):
                ...
                for n_steps in self.cfg.sampling.consistency_sampler.steps_to_test:
                    with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                        cons_x_t, cons_x_1 = self._sample_batch(
                            ...
                            data_device,
                            shared_noise_device,
                            shared_posterior_device,
                            labels=labels_device,
                            cfg_scale=cfg_scale,
                            posterior_sampler="consistency",
                            n_steps=n_steps,
                        )
                        cons_x_t = self._decode(
                            pl_module,
                            cons_x_t,
                            vae_batch_size=self.cfg.sampling.vae_batch_size,
                        )
                        cons_x_1 = self._decode(
                            pl_module,
                            cons_x_1,
                            vae_batch_size=self.cfg.sampling.vae_batch_size,
                        )
        ...
        for start in tqdm.tqdm(range(0, N_local, bs), desc="Batches"):
            ...
            with torch.no_grad():
                with torch.amp.autocast("cuda", enabled=False):
                    ...
                    latents = (
                        latents - pl_module.latents_bias
                    ) * pl_module.latents_scale
            ...
            alpha_t, beta_t = (
                broadcast_to_shape(alpha_t, x1_batch.shape),
                broadcast_to_shape(beta_t, x1_batch.shape),
            )
            ...
            model = (
                pl_module.model
                if kwargs.get("v_type", None) != "glass_flows"
                else pl_module.teacher_model
            )
    ...
    def _save_samples(self, pl_module, x_t, x_0, t_cond, sample_type, steps=None):
        save_dir = os.path.join(
            self.cfg.work_dir, f"samples_{pl_module.global_step}_ema"
        )
